...
class editartikel_F(FlaskForm):
    subjek = StringField('Subjek',validators=[DataRequired()])
    kategori= SelectField(u'Kategori artikel : ', choices=[('sayur','sayur'),('Action','Aksi'), ('weda','weda')], validators=[DataRequired()])
    detail_artikel= TextAreaField('Artikel')
    submit = SubmitField('Ubah')

class surat_F(FlaskForm):
    subjek = StringField('Pengirim : ',validators=[DataRequired()])
    kategori= SelectField(u'Kategori Surat : ', choices=[('Hmti','Hmti'),('Organisasi','Organisasi'), ('Kampus','Kamppus')], validators=[DataRequired()])
